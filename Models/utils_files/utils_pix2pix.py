import torch
from Models.config_files import config_pix2pix as config
...
def show_learnrate_reduction(lr_list, name="lr_reduction", lr=0.0002, epochs=200):
    epoch, lrs = zip(*lr_list)  # Trennen Sie die Epochen und die Lernraten in separate Listen

    plt.figure(figsize=(10, 6))  # Größe des Diagramms festlegen

    plt.plot(lrs, epoch, marker='o', linestyle='-', color='b')  # Plot der Lernraten über die Epochen
    ...
    #plt.show()  # Zeigen Sie das Diagramm an (optional)
