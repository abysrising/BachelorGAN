...
class ArtDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        real_image_array = np.array(real_image).astype(np.float32)
        sketch_array = np.array(sketch).astype(np.float32)

        augmentations = both_transform(image=sketch_array, image0=real_image_array)
